...
Armor chooseArmor(int minRange,int maxRange,vector <Armor> armorVector)
{
    Armor armorToReturn;
    int positionOfArmor = 0;

    positionOfArmor = rand() % maxRange + minRange;
...
void itemChoice(character player,vector <Item> &itemVector,
                ...
                vector <Armor> armorVector,vector <Weapon> weaponVector)
{
    srand(time(NULL));
    ...
    else if(roll >= minArmor && roll <= maxArmor)//armor
    {
        if(player.getLevel() >= 1 && player.getLevel() <= 9)
        {
            if(roll >= minLeather)
            {
                ...
                {
                    Armor newArmor;
                    newArmor = chooseArmor(0,4,armorVector);//0-first leather item in armorVector; 4 = last leather item

                    itemVector.push_back(newArmor);
                }
                else if (roll >= minMetal && roll <= maxLeather)
                {
...
//more than 1 type of each weapon?
// - if so, create vectors for each type of Weapon

//max level? can't do a lot with armor until we know max level
